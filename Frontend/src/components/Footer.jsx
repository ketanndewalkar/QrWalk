import React from "react";
import Logo from "../assests/Logo.png";
import { FaAngleDoubleUp } from "react-icons/fa";
import { FaFacebook } from "react-icons/fa";
import { FaXTwitter } from "react-icons/fa6";
import { FaInstagram } from "react-icons/fa";
import { FaLinkedinIn } from "react-icons/fa";

const Footer = () => {
  const handletop = () => {
    
    window.scrollTo({
      top: 0,
      behavior: "smooth",
    });
  };
  return (
    <div className="w-full h-80 bg-[#323232] rounded-t-lg px-30 py-5 mt-20 absolute">
      <div className="w-full h-full flex">
        <div className="w-1/2 h-full">
          <img src={Logo} className="h-20" />
          <p className="w-full px-14 font-bold text-[#cfcfcff0] tracking-widest mt-2">
            QR Walk isn’t just a service it’s a revolution in modern
            advertising. We redefine how businesses reach their audience by
            merging technology with creativity. Our platform enables brands to
            advertise in the most convenient, targeted, and effective way
            possible using QR technology.
          </p>
          <div className="border-1 ml-14 px-5 py-2 mt-5 w-fit flex items-center justify-center gap-3 hover:cursor-pointer rounded-lg border-[#cfcfcff0]" onClick={handletop}>
            <FaAngleDoubleUp className="text-[#cfcfcff0]" />{" "}
            <p className="font-bold text-[#cfcfcff0]">Back to Top</p>
          </div>
        </div>
        <div className="w-1/2 h-full mt-8">
          <div className="w-full h-fit flex border-b-2 border-[#cfcfcff0] p-3 gap-15">
            <FaFacebook className="text-[#cfcfcff0] text-3xl hover:scale-110 hover:cursor-pointer down" />
            <FaXTwitter className="text-[#cfcfcff0] text-3xl hover:scale-110 hover:cursor-pointer down" />
            <FaInstagram className="text-[#cfcfcff0] text-3xl hover:scale-110 hover:cursor-pointer down" />
            <FaLinkedinIn className="text-[#cfcfcff0] text-3xl hover:scale-110 hover:cursor-pointer down" />
          </div>
          <div className="w-full h-fit mt-13">
            <label className="text-[#cfcfcff0] font-bold text-[1.2rem]">
              Feedback :<br/>
              <input className="w-full border-b-1 border-white focus:outline-0 bg-[#3232323e]" />
              <br/>
              <button className="mt-5 w-fit px-4 py-1 border-1 border-white bg-[white] text-black hover:cursor-pointer">Submit</button>
            </label>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Footer;
